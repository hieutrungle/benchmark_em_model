...
class CombinedDataset(VisionDataset):
    ...
    Args:
        ...
        root2 (string, optional): Second root directory path.
    ...
    def __init__(
        ...
        targets_1 = [s[1] for s in samples_1]

        if root2 is not None:
            samples_2 = self.make_dataset(
                root2, class_to_idx, extensions, is_valid_file
            )
            targets_2 = [s[1] for s in samples_2]

            self.samples = samples_1 + samples_2
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        """
        # self.samples = [(path0, target0), (path1, target1), ...]
...
class CombinedImageDataset(CombinedDataset):
    """
...
class ImageConductivityDataset(data.Dataset):
    def __init__(
        self,  # class
        root: str,
        transform: Optional[Callable] = None,
        ...
        # self.output = preprocessing.MinMaxScaler().fit_transform(self.output)
        # self.output = (self.output - 2.474435)/13.61189

        print(f"Found {len(self.image_files)} images in {self.image_dir}")
        print(f"Found {len(self.output)} outputs in {current_file}")
        # print(self.output)
